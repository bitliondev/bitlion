...
pkgdatadir = $(datadir)/bitlioncoincore
pkgincludedir = $(includedir)/bitlioncoincore
pkglibdir = $(libdir)/bitlioncoincore
pkglibexecdir = $(libexecdir)/bitlioncoincore
...
CONFIG_HEADER = $(top_builddir)/src/config/bitlioncoin-config.h
CONFIG_CLEAN_FILES = libbitlioncoinconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libbitlioncoinconsensus.pc.in \
	...
	$(top_srcdir)/src/config/bitlioncoin-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/bitlioncoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/bitlioncoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/bitlioncoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/bitlioncoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = bitlioncoin-cli
BITCOIN_DAEMON_NAME = bitlioncoind
BITCOIN_GUI_NAME = bitlioncoin-qt
BITCOIN_TX_NAME = bitlioncoin-tx
...
COPYRIGHT_HOLDERS_FINAL = The bitlioncoin Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = bitlioncoin Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/bitlioncoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = bitlioncoincore
PACKAGE_BUGREPORT = https://github.com/bitlioncoinpay/bitlioncoin/issues
PACKAGE_NAME = bitlioncoin Core
PACKAGE_STRING = bitlioncoin Core 1.0.0
PACKAGE_TARNAME = bitlioncoincore
PACKAGE_URL = https://bitlioncoin.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/bitlioncoin
abs_srcdir = /tmp/bitlioncoin
abs_top_builddir = /tmp/bitlioncoin
abs_top_srcdir = /tmp/bitlioncoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/bitlioncoin/build-aux/install-sh
...
pkgconfig_DATA = libbitlioncoinconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/bitlioncoin-cli.bash-completion \
	       $(top_srcdir)/contrib/bitlioncoin-tx.bash-completion \
	       $(top_srcdir)/contrib/bitlioncoind.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_bitlioncoin_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_bitlioncoin_coverage.info test_bitlioncoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/bitlioncoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/bitlioncoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/bitlioncoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/bitlioncoin-config.h
$(top_srcdir)/src/config/bitlioncoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/bitlioncoin-config.h src/config/stamp-h1
libbitlioncoinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libbitlioncoinconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/bitlioncoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/bitlioncoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/bitlioncoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_bitlioncoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_bitlioncoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_bitlioncoin_filtered.info: test_bitlioncoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_bitlioncoin_filtered.info
...
#test_bitlioncoin_coverage.info: baseline_filtered_combined.info test_bitlioncoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitlioncoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_bitlioncoin_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitlioncoin_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bitlioncoin.coverage/.dirstamp:  test_bitlioncoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_bitlioncoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_bitlioncoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
