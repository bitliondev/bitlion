...
const char * const BITCOIN_CONF_FILENAME = "bitlioncoin.conf";
const char * const BITCOIN_PID_FILENAME = "bitlioncoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitlioncoin" is a composite category enabling all bitlioncoin-related debug output
                if(ptrCategory->count(std::string("bitlioncoin"))) {
...
#else
    const char* pszModule = "bitlioncoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bitlioncoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bitlioncoin
    // Mac: ~/Library/Application Support/bitlioncoin
    // Unix: ~/.bitlioncoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bitlioncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bitlioncoin";
#else
    // Unix
    return pathRet / ".bitlioncoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitlioncoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitlioncoin.conf
