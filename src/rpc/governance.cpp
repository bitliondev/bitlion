...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in bitlioncoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("bitlioncoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "bitlioncoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "bitlioncoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "bitlioncoin",               "gobject",                &gobject,                true,  {} },
    { "bitlioncoin",               "voteraw",                &voteraw,                true,  {} },
